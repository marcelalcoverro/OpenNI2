...
JNIEXPORT jint JNICALL Java_org_openni_NativeMethods_oniDeviceOpen__Ljava_lang_String_2Lorg_openni_Device_2I
(JNIEnv *env, jclass, jstring uriStrObj, jobject device, jint fd)
  {
	  OniDeviceHandle handle;
	  const char * str = env->GetStringUTFChars(uriStrObj, JNI_FALSE);
	  int status = oniDeviceOpen(str, &handle, fd);
...
JNIEXPORT jint JNICALL Java_org_openni_NativeMethods_oniDeviceOpen__Lorg_openni_Device_2I
	(JNIEnv *env, jclass, jobject device, jint fd)
{
	OniDeviceHandle handle;
	int status = oniDeviceOpen(ANY_DEVICE, &handle, fd);
	jclass deviceCls = env->FindClass("org/openni/Device");
	jfieldID fieldID = env->GetFieldID(deviceCls, "mDeviceHandle", "J");
	env->SetLongField(device, fieldID, (long)handle);
	return status;
