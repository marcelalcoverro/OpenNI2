...
struct Funcs
{
	...
	// As Device
	void* (ONI_C_DECL* oniDriverDeviceOpen)(const char* uri, const char* mode, int fd);
...
public:
	...
	void* deviceOpen(const char* uri, const char* mode, int fd) const
	{
		return (*funcs.oniDriverDeviceOpen)(uri, mode, fd);
	}
